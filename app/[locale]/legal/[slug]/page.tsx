...
export default async function LegalPage({ params }: { params: Promise<{ locale: string; slug: string }> }) { const { locale, slug } = await params; if (!isLocale(locale)) notFound(); const doc = legalDocuments.find(x => x.slug === slug); if (!doc) notFound(); const ar = locale === "ar"; return <><SiteHeader locale={locale} /><main className="legal-page container"><Link className="back-link" href={`/${locale}`}>← {ar ? "الرئيسية" : "Home"}</Link><h1>{doc.title[locale]}</h1><p className="legal-summary">{doc.summary[locale]}</p>{doc.sections.map(section => <section key={section.title[locale]}><h2>{section.title[locale]}</h2>{section.paragraphs[locale].map(p => <p key={p}>{p}</p>)}</section>)}<section><h2>{ar ? "مراجع قانونية" : "Legal references"}</h2><ul className="source-list">{legalSources.map(source => <li key={source.url}><a href={source.url} target="_blank" rel="noreferrer">{source.label}</a></li>)}</ul></section><p className="review-date">{ar ? "آخر تحديث: 13 أغسطس 2026" : "Last updated: 13 August 2026"}</p></main><SiteFooter locale={locale} /></>; }
