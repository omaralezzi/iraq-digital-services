    ...
    <div className="container hero-layout">
      <div className="hero-copy">
        ...
        <div className="hero-actions">
          ...
          <Link className="button ghost" href={`/${locale}/demos`}>{ar ? "جرّب النماذج" : "Try the demos"}</Link>
        </div>
        <div className="hero-experience"><span className="hero-experience-years" aria-hidden="true"><b>25</b><small>{ar ? "عامًا" : "years"}</small></span><i aria-hidden="true"></i><p><strong>{ar ? "خبرة برمجية" : "Programming experience"}</strong><small>{ar ? <>منذ عام <b>2001</b></> : <>since <b>2001</b></>}</small></p></div>
        <div className="trust-row"><span><i>✓</i>{ar ? "تواصل بالعربية" : "Arabic communication"}</span><span><i>✓</i>{ar ? "نطاق واضح" : "Clear scope"}</span><span><i>✓</i>{ar ? "قابل للتوسع" : "Built to scale"}</span></div>
      </div>
      <div className="hero-scene" aria-label={ar ? "مشهد توضيحي يربط موقعًا وتطبيقًا ولوحة إدارة" : "Illustration connecting a website, app and dashboard"}>
        ...
        <div className="flow-chip chip-one"><i>✓</i>{ar ? "تم تسجيل الطلب" : "Request captured"}</div><div className="flow-chip chip-two"><i>↯</i>{ar ? "الأتمتة تعمل" : "Automation live"}</div>
        <div className="data-dot d1"></div><div className="data-dot d2"></div><div className="data-dot d3"></div>
      </div>
    </div>
    <div className="container hero-foot"><span>{ar ? "مزوّد الخدمة والطرف المتعاقد" : "Service provider and contracting party"}</span><b>{siteSettings.provider.legalName}</b><span>•</span><span>Düsseldorf, Germany</span><span>→</span><span>Iraq</span></div>
  </section>;
}
